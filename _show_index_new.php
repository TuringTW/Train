...
<?php 
	...
	function stationfindtrain($route,$arrtime,$fromstation){
		...
		if (!empty($route)) {
			$length = count($route);
			$travel_time = strtotime($route[$length-1]['arrtime'])-strtotime($route[0]['deptime']);
			$total_time = strtotime($route[0]['arrtime'])-strtotime($strtime);
			$total_transfer = $length/2-1;
			$grade = $travel_time*0.5+$total_time*1+$total_transfer*10*60*2;
		}
		if ($tostation == $fromstation) {
			if (count($allroute)>=$searchresult) {
				if ($allroute[$searchresult-1][0]['grade']>$grade) {
					$perform = array('travel_time' => $travel_time, 'total_time' => $total_time,'total_transfer' => $total_transfer,'grade' => $grade);
					array_pop($allroute);
					$temp = array(0 => $perform, 1 => $route);	
					// 依評分排序
					sort_route($temp);
				}
			}else{
		...
		}else if($total_transfer<1){	
			$sql = "SELECT  `timetable`.`ttable_id`,`timetable`.`train_id`,`timetable`.`order`,`timetable`.`deptime`,`timetable`.`arrtime` ,`timetable`.`station_id` from `timetable` LEFT JOIN `train` on `train`.`train_id` = `timetable`.`train_id` where `timetable`.`station_id` = '$fromstation' and UNIX_TIMESTAMP(`deptime`)-UNIX_TIMESTAMP('$arrtime')>0  and UNIX_TIMESTAMP(`deptime`)-UNIX_TIMESTAMP('".date('Y-m-d G:i:s',strtotime($strtime." +$waithour hours"))."') <0 and `train`.`train_id` NOT IN (".join(',',trainp($route)).") order by `train`.`class`,`deptime`";
			if (!empty($allroute)) {
	...
	function trainfindstation($route,$train_id,$order){
		...
		$sql = "SELECT  `timetable`.`ttable_id`,`timetable`.`train_id`,`timetable`.`deptime`,`timetable`.`arrtime` ,`timetable`.`station_id` from `timetable` LEFT JOIN `station` on `station`.`station_id` = `timetable`.`station_id` where `train_id` = '$train_id' && `order` > '$order' and UNIX_TIMESTAMP(`arrtime`)-UNIX_TIMESTAMP('".date('Y-m-d G:i:s',strtotime($strtime." +$waithour hours"))."') <0 and `timetable`.`station_id` NOT IN (".join(',',$pre_station).") order by `station`.`class` DESC,`arrtime` ";
